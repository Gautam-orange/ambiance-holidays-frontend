...
export default function EditTour() {
  ...
  const [form, setForm] = useState({
    title: '', category: 'LAND', region: 'NORTH', duration: 'FULL_DAY',
    durationHours: '', description: '', adultPriceCents: '', childPriceCents: '',
    infantPriceCents: '0', minPax: '1', maxPax: '20',
    includes: '', excludes: '', importantNotes: '', coverImageUrl: '',
    status: 'ACTIVE', availabilityMode: 'always', theme: 'NATURE',
  });
  ...
  useEffect(() => {
    ...
    adminGetTour(id)
      .then((res: any) => {
        ...
        setForm({
          title: t.title ?? '',
          category: t.category ?? 'LAND',
          region: t.region ?? 'NORTH',
          duration: t.duration ?? 'FULL_DAY',
          durationHours: t.durationHours != null ? String(t.durationHours) : '',
          description: t.description ?? '',
          adultPriceCents: t.adultPriceCents != null ? String(t.adultPriceCents / 100) : '',
          childPriceCents: t.childPriceCents != null ? String(t.childPriceCents / 100) : '',
          infantPriceCents: t.infantPriceCents != null ? String(t.infantPriceCents / 100) : '0',
          minPax: t.minPax != null ? String(t.minPax) : '1',
          maxPax: t.maxPax != null ? String(t.maxPax) : '20',
        ...
        setItinerary((t.itineraryStops ?? []).map((s: any) => ({
          stopTime: s.stopTime ?? '', title: s.title ?? '', description: s.description ?? '',
        })));
        setPickups((t.pickupZones ?? []).map((p: any) => ({
          zoneName: p.zoneName ?? '',
          extraCents: p.extraCents != null ? String(p.extraCents / 100) : '0',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.response?.data?.error?.message ?? 'Failed to update tour');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-display font-bold text-slate-800">Edit Tour</h2>
          <p className="text-slate-500 mt-1">Update excursion package details</p>
        </div>
      </div>
