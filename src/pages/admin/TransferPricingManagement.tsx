...
type TierFormData = Omit<Tier, 'id'> & { includesText: string; excludesText: string };

// AM_042: TierForm extracted to a top-level component. Previously declared
// inline as `const TierForm = …` inside the parent — each parent re-render
// created a fresh component reference, so React unmounted+remounted the
// inputs on every keystroke and the cursor jumped to the end.
function TierForm({
  id, form, setForm, saving, onSave, onCancel,
}: {
  id?: string;
  form: TierFormData;
    ...
    <div className="p-4 bg-slate-50 rounded-xl border border-brand-primary/20 space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onCancel} className="px-3 py-2 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-100">
...
export default function TransferPricingManagement() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {showAdd && (
        <div className="mb-4">
          <TierForm form={form} setForm={setForm} saving={saving} onSave={handleSave} onCancel={cancelEdit} />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {tiers.map(tier => (
            <div key={tier.id}>
              {editId === tier.id ? (
                <TierForm id={tier.id} form={form} setForm={setForm} saving={saving} onSave={handleSave} onCancel={cancelEdit} />
              ) : (
